...
BaseRoulante::BaseRoulante()
{
	...
	kPRotation = 0.075; //0.050 pour cot�s et 0.075 pour milieu
	kIRotation = 0.00003;
	kDRotation = 0.28;
}
...
{
	if(etatGachette && !etatPrecedentGachette) //Si la gachette est appuy�e et qu'elle ne l'etait pas avant
	{
	...
	else if (!etatGachette) //Si la gachette n'est pas appuy�es
	{
		etatPrecedentGachette = false;
	}
}
...
double BaseRoulante::rotation(const int angle_consigne)
{
	...
	return erreur; // Erreur en degr�s
}
